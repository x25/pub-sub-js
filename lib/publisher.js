...
var Chunks = require('./chunks').Chunks;
...
Publisher.prototype.send = function (channel, data) {
	...
	var overSizedSockets = [];

	for (var s in this.channels[channel]) {
		var socket = this.channels[channel][s];

		if (socket instanceof net.Socket) {
			if (socket.bufferSize > 10*1024*1024) {
				console.log('oversized socket!');
				if (overSizedSockets.indexOf(socket) === -1) {
					overSizedSockets.push(socket);
				}
...
try {
	...
	for (var ss in overSizedSockets)
		overSizedSockets[ss].destroy();
};
...
Publisher.prototype.connect = function (port, host) {
	...
	var buffer = new Chunks('\n');
	buffer.on('chunk', function(chunk) {
